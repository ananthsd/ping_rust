...
fn main() {
    ...
    let ttl = 50;
    let sequence_num = 0;
    let packet = get_ipv4_packet(&mut ipv4_buf, &mut icmp_buf, ttl, sequence_num, destination);
    ...
    loop{
        ...
        match next {
            Ok((packet,addr))=>{
                println!("we got a packet: {:?} from {}",IcmpPacket::new(packet.payload()), addr);
//                println!("packet code: {:?}",packet.get_icmp_type());
//                return;
            },
            Err(e)=>{
...
fn get_payload(buf: &mut [u8], sequence_num:u16) -> MutableEchoRequestPacket {
    use pnet::packet::icmp::{IcmpPacket, IcmpTypes, IcmpCode};
    use pnet::util::checksum;
...
fn get_ipv4_packet<'a>(packet_buf: &'a mut [u8], payload_buf: &'a mut[u8], ttl:u8, sequence_num:u16, destination: Ipv4Addr) -> MutableIpv4Packet<'a> {
...
//    packet.set_source(source);
    ...
    packet.set_payload(get_payload(payload_buf, 121).packet_mut());
    packet
}